...
| 7a.31a | Keep only **`Nicole Park`**. In a **mention** (not the subject), say "Nicole" | Silently linked to her — **no question.** A single overlap match for a mention keeps the same light treatment a single exact match always had; only the subject path requires confirming a unique match | ⬜ |
