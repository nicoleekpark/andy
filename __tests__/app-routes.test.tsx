...
describe("app route tree", () => {
  ...
  test("should resolve /capture as a route distinct from /profile/[id]/capture, both rendering the capture screen", async () => {
    ...
    // a note recorded on Jiseon's page ended up asking who it was about.
    mockCaptureQueries({
      profile: { _id: "contact-1", name: "Jiseon", entityType: "person", tags: [], autoCreated: false },
      notes: [],
      mentionedIn: [],
      mentionedInTotal: 0,
    });
    ...
    expect(
      screen.getByText("Tap record. This note goes to Jiseon, whoever else comes up."),
    ).toBeTruthy();
  });
});
