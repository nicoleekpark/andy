...
| **`QA.md`** | 릴리스 전, 그리고 화면을 바꾼 PR을 머지하기 전 | 수동 검증 목록 — **jest·vitest가 구조적으로 못 하는** 확인들. 두 러너는 시뮬레이터·마이크·네트워크·배포 중 무엇도 건드리지 않는다. Day 3에 앱을 써서 나온 버그 보고 20건 중 두 러너가 잡을 수 있었던 것은 0건이었다. |
